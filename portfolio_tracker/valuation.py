...
from .storage import default_db_path, get_market_series_latest, get_market_series_points, get_structured_events
...
class MarkToMarketEngine(BaseValuationEngine):
    ...
    def _load_nav(self, asset_id: str, target_date: date) -> Optional[dict[str, Any]]:
        latest = get_market_series_latest(
            default_db_path(self.data_dir),
            kind="uc",
            identifier=asset_id,
...
class HybridEngine(BaseValuationEngine):
    ...
    def _estimative_valuation(self, asset: Asset, position: Position, valuation_date: date) -> ValuationResult:
        nav_data = MarkToMarketEngine(self.data_dir)._load_nav(asset.asset_id, valuation_date)
        if nav_data:
            nav_value = nav_data["value"]
            nav_date = nav_data["date"]
...
class DeclarativeEngine(BaseValuationEngine):
    ...
    def _load_declared_rates(self, asset_id: str) -> dict[int, dict[str, Any]]:
        ...
        for p in points:
            year = date.fromisoformat(str(p["date"])).year
            rates[year] = {"rate": p["value"], "source": p.get("source") or "unknown", "date": p["date"]}
        return rates
...
class EventBasedEngine(BaseValuationEngine):
    def valuate(self, asset: Asset, position: Position, valuation_date: Optional[date] = None) -> ValuationResult:
        ...
        if units_held_yaml is not None:
            ...
            if units_held_float < 0.01 and current_projection.is_closed and sell_date is not None and sell_date <= val_date:
    ...
    def _load_event_file(self, asset_id: str) -> tuple[list[ValuationEvent], list[ValuationEvent]]:
        realized_raw, expected_raw = get_structured_events(
            default_db_path(self.data_dir),
            asset_id=asset_id,
        )
        return (
            self._parse_events_list(realized_raw, expected=False),
            self._parse_events_list(expected_raw, expected=True),
        )
