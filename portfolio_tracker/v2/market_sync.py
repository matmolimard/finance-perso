...
def _nav_file(market_data_dir: Path, identifier: str) -> Path:
    return Path(market_data_dir) / f"nav_{identifier}.yaml"


def upsert_nav_point(*, market_data_dir: Path, identifier: str, point: NavPoint) -> tuple[Path, bool]:
    market_data_dir = Path(market_data_dir)
    market_data_dir.mkdir(parents=True, exist_ok=True)
    nav_file = _nav_file(market_data_dir, identifier)
    payload = yaml.safe_load(nav_file.read_text(encoding="utf-8")) if nav_file.exists() else {}
    payload = payload or {}
    ...
    new_history = []
    for row in history:
        if str(row.get("date")) == point.point_date.isoformat():
            replaced = True
            existing = (
            ...
            incoming = (float(point.value), point.currency, point.source)
            ...
            new_history.append(
                {
                    ...
                    **({"source": point.source} if point.source else {}),
                }
            )
        else:
    ...
    new_history.sort(key=lambda row: str(row.get("date") or ""))
    payload["nav_history"] = new_history
    nav_file.write_text(yaml.safe_dump(payload, sort_keys=False, allow_unicode=True), encoding="utf-8")
    return nav_file, True


def upsert_nav_history(*, market_data_dir: Path, identifier: str, points: list[NavPoint]) -> tuple[Path, int]:
    market_data_dir = Path(market_data_dir)
    market_data_dir.mkdir(parents=True, exist_ok=True)
    nav_file = _nav_file(market_data_dir, identifier)
    payload = yaml.safe_load(nav_file.read_text(encoding="utf-8")) if nav_file.exists() else {}
    ...
    changed = 0
    for point in points:
        key = point.point_date.isoformat()
        incoming = {
            "date": key,
            "value": float(point.value),
            "currency": point.currency,
    ...
    payload["nav_history"] = [existing[key] for key in sorted(existing)]
    nav_file.write_text(yaml.safe_dump(payload, sort_keys=False, allow_unicode=True), encoding="utf-8")
    return nav_file, changed
...
def fetch_nav_history_for_asset_id(
    *,
    ...
    if morningstar_secid:
        return _fetch_morningstar_history(str(morningstar_secid), start_date, end_date, currency=currency)
    ...
    if single and start_date <= single.nav_date <= end_date:
        return [single]
    return []
    ...
    def _backfill_asset(asset) -> None:
        try:
            ...
            if not fetched_points:
                ...
                return
            _, changed_count = upsert_nav_history(
                market_data_dir=market_data_dir,
                identifier=asset.asset_id,
                points=[
                    NavPoint(
...
def fetch_solactive_indexhistory(url: str, identifier: str, *, headless: bool = False) -> FetchResult:
    ...
    text = _http_get_text(
        url,
        headers={"User-Agent": "portfolio-tracker/1.0 (personal project)", "Accept": "application/json,text/plain,*/*", "Referer": referer},
        headless=headless,
    )
    try:
    ...
    return FetchResult(source="solactive", identifier=identifier, points=points, metadata={"url": url, "points_count": len(points)})
