...
from .storage import get_structured_events
...
class V2Runtime:
    ...
    def __init__(self, data_dir: Path, *, db_path: Path | None = None, include_db_overlay: bool = True):
        ...
        self.portfolio = PortfolioData(self.data_dir, db_path=db_path, include_db_overlay=include_db_overlay)
        ...
        self.underlyings_provider = UnderlyingProvider(self.market_data_dir, db_path=self.portfolio.db_path)
        self.rates_provider = RatesProvider(self.market_data_dir, db_path=self.portfolio.db_path)
        ...
        for position in positions:
            ...
            if perf_amt is not None and months > 0:
                ...
                if years_from_months > 0:
                    perf_annualized = self._annualize_pct_return(perf_amt, years_from_months)
    ...
    def _load_structured_coupon_tracking(self, *, asset_id: str, engine: Any, valuation_date: date) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        try:
            raw_realized, raw_expected = get_structured_events(
                self.portfolio.db_path,
                asset_id=asset_id,
            )
            realized_events = []
                ...
                try:
                    ...
                    if float(invested_amount) != 0:
                        ...
                        if months_until_next_obs > 0:
                            ...
                            perf_if_strike_next_annualized = self._annualize_pct_return(
                                perf_if_strike_next,
                                years_until_next,
                            )
                except Exception:
    ...
    @staticmethod
    def _annualize_pct_return(total_return_pct: float | None, years: float | None) -> float | None:
        if total_return_pct is None or years is None or years <= 0:
            return None
        base = 1.0 + float(total_return_pct) / 100.0
        if base <= 0:
            return None
        try:
            return ((base ** (1.0 / float(years))) - 1.0) * 100.0
    ...
    def calculate_performance_metrics(
        ...
        if days_elapsed > 0:
            years_real = days_elapsed / 365.25
            if years_real > 0:
                result["perf_annualized"] = self._annualize_pct_return(perf, years_real)
        return result
