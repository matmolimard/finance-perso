    ...
    *,
    ...
    if bootstrap or not db_path.exists():
        ...
        import_summary = {
            ...
            "imported": {
                ...
                "portfolio_assets_seeded": 0,
                "portfolio_positions_seeded": 0,
                "portfolio_lots_seeded": 0,
                "portfolio_positions_reconstructed_from_pdf": 0,
                "portfolio_lots_reconstructed_from_pdf": 0,
                "snapshot_position_links_backfilled": 0,
                "snapshot_positions_imported": 0,
                "snapshot_operations_imported": 0,
                "snapshot_operation_legs_imported": 0,
    ...
    with connect(db_path) as conn:
        ...
        structured_official_by_snapshot = {
            ...
            for row in conn.execute(
                """
                SELECT snapshot_id, SUM(official_value) AS total
                FROM snapshot_positions
                WHERE asset_type = 'structured_product'
    ...
    for row in contracts:
        ...
        latest_validated_snapshot = next(
            (snap for snap in reversed(timeline) if str(snap.get("status")) == "validated"),
            None,
        )
        ...
        if latest_validated_snapshot is None:
            reconciliation = {
                "status": "unavailable",
                "message": "Aucun snapshot validé",
                "reference_date": None,
                "gap_amount": None,
                "gap_pct": None,
            }
        else:
            reference_value = float(latest_validated_snapshot["official_total_value"] or 0.0)
            gap_amount = current_value - reference_value
            gap_pct = (gap_amount / reference_value * 100.0) if reference_value else None
            reconciliation = {
                "status": "ok" if abs(gap_amount) < 1.0 else "warning",
                "message": "Rapprochement vs snapshot validé",
                "reference_date": latest_validated_snapshot["reference_date"],
        ...
        contract_cards.append(
            {
                ...
                "latest_structured_model_gap": (
                    {
                        "value": last_snapshot.get("structured_model_gap_value"),
                        "pct": last_snapshot.get("structured_model_gap_pct"),
                        "official_structured_value": last_snapshot.get("official_structured_value"),
                        "model_structured_value": last_snapshot.get("model_structured_value"),
                    }
                    if last_snapshot
                    else None
                ),
