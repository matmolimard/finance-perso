...
class UnderlyingProvider:
    ...
    def get_history(self, identifier: str, start_date: date | None = None, end_date: date | None = None) -> list[UnderlyingPoint]:
        ...
        return [
            UnderlyingPoint(point_date=datetime.fromisoformat(str(row["date"])).date(), value=float(row["value"]))
            for row in points
        ]
    ...
    def upsert_history(
        ...
        *,
        ...
        return upsert_market_series_points(
            ...
            points=[{"date": point_date, "value": value} for point_date, value in points],
            source=source,
            source_url=(extra or {}).get("url"),
            metadata=extra,
        )
...
class RatesProvider:
    ...
    def get_data(self, identifier: str, data_type: str = "rate", target_date: date | None = None) -> dict[str, Any] | None:
        ...
        if latest is None:
            return None
        return {"date": latest["date"], "value": latest["value"], "source": latest.get("source")}
